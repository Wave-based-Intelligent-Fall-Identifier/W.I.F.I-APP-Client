...
- 서버(FastAPI)는 별도로 상태/이력 저장·관리·푸시(FCM 등)를 담당한다. 앱은 서버 REST를 **필수로 쓰지 않는다**(MVP). 추후 이력은 서버 REST로 보강(§8). 사용자 인증은 도입하지 않음(§8).
...
- ~~회원가입 / 로그인(인증)~~ — **범위 제외 결정**(2026-06-16). 앱은 브로커에 익명 접속하며, 접근 통제는 네트워크/브로커 ACL 레벨에서 처리한다. 앱 단 사용자 인증은 도입하지 않는다.
